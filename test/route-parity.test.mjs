...
test('Fresh Shipping Box contract requires draft confirmation measurement and cold holding',()=>{const required='  /api/transforms: {post: {summary: Apply identity-preserving WASH or SLICE; observation weights never replace inventory}}\n  /api/sorting: {post: {summary: Require a distinct empty physical output container and SORT_OUTPUT_CONTAINER_SCAN_REQUIRED}}',incomplete=`${required}\n  /api/fresh-shipping-boxes: {post: {summary: Create READY_TO_SHIP box immediately}}`;assert.throws(()=>assertCriticalOperationContracts(incomplete),/draft confirmation measurement/);assert.doesNotThrow(()=>assertCriticalOperationContracts(`${required}\n  /api/fresh-shipping-boxes: {post: {summary: Backward-compatible atomic draft plus confirm with measurementId}}\n  /api/fresh-shipping-boxes/drafts: {post: {summary: Reserve aggregated net-lot allocations with FRESH_SHIPPING_BOX_FULL}}\n  /api/fresh-shipping-boxes/{id}/confirm: {post: {summary: Enter LABEL_PENDING}}\n  /api/fresh-shipping-boxes/{id}/cold-holding: {post: {summary: Move LABEL_PRINTED to READY_TO_SHIP}}`))});
