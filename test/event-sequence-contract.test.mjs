...
test('dead-letter outbox events expose retry state and a requeue route',()=>{assert.match(spec,/\/api\/outbox:.*nextAttemptAt.*deadLetterReason/);assert.match(spec,/\/api\/outbox\/\{eventId\}\/requeue:.*DEAD_LETTER.*PENDING/)});
