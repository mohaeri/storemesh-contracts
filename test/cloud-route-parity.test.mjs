...
test('cloud contract has parity with the real aggregator route source',()=>{
  assert.equal(assertRouteParity(server,contract),4);
});

test('cloud authentication and response shapes are published',()=>{
  for(const token of['X-Cloud-Report-Key','X-Site-Code','X-Site-Key','X-Site-Key-Id','CONNECTED, STALE, PENDING','accepted:','duplicates:','rejected:','checkpoint:'])assert.match(contract,new RegExp(token));
});
